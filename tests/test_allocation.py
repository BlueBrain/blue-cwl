...
def test_allocation__shell_command():
    ...
    allocation = tested.Allocation(config=config)
    res = allocation.shell_command(cmd="echo X")
    assert res == "export A=X B=Y && salloc -J F salloc -N 1 -n 2 --exclusive srun sh -c 'echo X'"
    config["skip_srun"] = True
    allocation = tested.Allocation(config=config)
    res = allocation.shell_command(cmd="echo X")
    assert res == "export A=X B=Y && salloc -J F salloc -N 1 -n 2 --exclusive  sh -c 'echo X'"
