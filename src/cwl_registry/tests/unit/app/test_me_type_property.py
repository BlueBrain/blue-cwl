...
from cwl_registry import Variant, utils
...
def _create_me_type_densities(output_file):
    ...
    utils.write_json(filepath=output_file, data=dataset)


def _mock_me_type_densities_resource(resource_dir):

    out_dir = utils.create_dir(resource_dir / "me-type-densities")
...
def test_me_type_property__extract(tmp_path):
    ...
    out = utils.create_dir(tdir / "out")
    resources_dir = utils.create_dir(tdir / "resources")

    mocked_resources = {
        "brain-region-id": Mock(label="root"),
        "variant-config-id": None,
        **_mock_atlas_resource(utils.create_dir(resources_dir / "atlas")),
        **_mock_me_type_densities_resource(resources_dir),
    }

    with (
        patch("cwl_registry.app.me_type_property.get_kg_forge") as patched_get_kg_forge,
...
def test_me_type_property__transform(tmp_path):
    ...
    variant_dir = utils.create_dir(tdir / "variant")
    variant = _mock_variant(variant_dir)

    stage_dir = utils.create_dir(tdir / "stage")
    ...
    assert utils.load_yaml(expected_bioname_dir / "MANIFEST.yaml") == expected_manifest

    # check cell composition
    assert utils.load_yaml(expected_bioname_dir / "cell_composition.yaml") == {
        "version": "v2",
        "neurons": [
            {
...
def test_me_type_property__generate(tmp_path):
    ...
    build_dir = out / "build"
    config = utils.load_json(build_dir / "config.json")
...
def test_me_type_property__register():
    ...
    # not much to do
    with (
        patch("cwl_registry.app.me_type_property.get_kg_forge"),
