...
from cwl_luigi.nexus import GroupedDataset, get_resource, read_json_file_from_resource
from cwl_luigi.utils import write_json
...
def stage_mtype_densities(
    forge,
    resource_id: str,
    output_file: Path,
):
    """Stage mtype densities resource."""
    resource = get_resource(forge=forge, resource_id=resource_id)
    grouped_dataset = GroupedDataset.from_resource(forge=forge, resource=resource)
    ...
    for identifier, group in grouped_dataset.groups.items():
        ...
        mtype_groups[identifier] = {
            ...
            "path": _remove_prefix("file://", distribution.atLocation.location),
        }
    ...
    def _get_etype_ratio_from_resource(resource):
        ...
        for e in resource.annotation:
            if e.type == ["Annotation", "ETypeAnnotation"]:
                etype = e.hasBody.label
            elif e.type == ["Annotation", "MTypeAnnotation"]:
                mtype = e.hasBody.label

        assert mtype
        assert etype

        ratio = resource.series[0].value

        return {"etype": etype, "ratio": ratio}
